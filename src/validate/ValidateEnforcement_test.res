...
test("validateEnforcement: accepts a valid enforcement object", () => {
  let dict: dict<JSON.t> = %raw(`{enforcement: {mode: "advisory"}}`)
  assertionNoThrow(~operator="valid enforcement", () =>
    ValidateEnforcement.validateEnforcement(dict)
  )
})

test("validateEnforcement: passes silently when enforcement is absent", () => {
  let dict: dict<JSON.t> = %raw(`{activePreset: "default"}`)
  assertionNoThrow(~operator="absent enforcement", () =>
    ValidateEnforcement.validateEnforcement(dict)
  )
})
...
test("validateEnforcementMode: accepts mode 'off'", () => {
  let dict: dict<JSON.t> = %raw(`{mode: "off"}`)
...
test("validateEnforcementMode: accepts mode 'advisory'", () => {
  let dict: dict<JSON.t> = %raw(`{mode: "advisory"}`)
  assertionNoThrow(~operator="mode advisory", () =>
    ValidateEnforcement.validateEnforcementMode(dict)
  )
})

test("validateEnforcementMode: accepts mode 'enforced'", () => {
  let dict: dict<JSON.t> = %raw(`{mode: "enforced"}`)
  assertionNoThrow(~operator="mode enforced", () =>
    ValidateEnforcement.validateEnforcementMode(dict)
  )
})
...
test("validateEnforcementVerify: accepts a valid verify block", () => {
  let dict: dict<JSON.t> = %raw(`{verify: {require: "whenDoDPresent"}}`)
  assertionNoThrow(~operator="valid verify", () =>
    ValidateEnforcement.validateEnforcementVerify(dict)
  )
})
...
test("validateEnforcementVerify: accepts verify with all optional fields", () => {
  let dict: dict<
    JSON.t,
  > = %raw(`{verify: {require: "always", preferDeterministic: true, graderPolicy: "atLeastProducerTier"}}`)
  assertionNoThrow(~operator="full verify", () =>
    ValidateEnforcement.validateEnforcementVerify(dict)
  )
})
...
test("validateEnforcementVerify: passes silently when verify is absent", () => {
  let dict: dict<JSON.t> = %raw(`{mode: "off"}`)
  assertionNoThrow(~operator="absent verify", () =>
    ValidateEnforcement.validateEnforcementVerify(dict)
  )
})
...
test("validateEnforcementEscalate: accepts a valid escalate block with ladder", () => {
  let dict: dict<JSON.t> = %raw(`{escalate: {ladder: ["tier1", "tier2"]}}`)
  assertionNoThrow(~operator="valid escalate", () =>
    ValidateEnforcement.validateEnforcementEscalate(dict)
  )
})

test("validateEnforcementEscalate: accepts floorTier as string", () => {
  let dict: dict<JSON.t> = %raw(`{escalate: {floorTier: "fast"}}`)
  assertionNoThrow(~operator="floorTier string", () =>
    ValidateEnforcement.validateEnforcementEscalate(dict)
  )
})

test("validateEnforcementEscalate: accepts floorTier as null", () => {
  let dict: dict<JSON.t> = %raw(`{escalate: {floorTier: null}}`)
  assertionNoThrow(~operator="floorTier null", () =>
    ValidateEnforcement.validateEnforcementEscalate(dict)
  )
})
...
test("validateEnforcementEscalate: passes silently when escalate is absent", () => {
  let dict: dict<JSON.t> = %raw(`{mode: "off"}`)
  assertionNoThrow(~operator="absent escalate", () =>
    ValidateEnforcement.validateEnforcementEscalate(dict)
  )
})

test("validateEnforcementEscalate: accepts zero maxAttemptsPerTier", () => {
  let dict: dict<JSON.t> = %raw(`{escalate: {maxAttemptsPerTier: 0}}`)
  assertionNoThrow(~operator="zero maxAttemptsPerTier", () =>
    ValidateEnforcement.validateEnforcementEscalate(dict)
  )
})
...
test("validateEnforcementPerTier: accepts a valid perTier object", () => {
  let dict: dict<JSON.t> = %raw(`{perTier: {fast: "off", heavy: "enforced"}}`)
  assertionNoThrow(~operator="valid perTier", () =>
    ValidateEnforcement.validateEnforcementPerTier(dict)
  )
})

test("validateEnforcementPerTier: passes silently when perTier is absent", () => {
  let dict: dict<JSON.t> = %raw(`{mode: "off"}`)
  assertionNoThrow(~operator="absent perTier", () =>
    ValidateEnforcement.validateEnforcementPerTier(dict)
  )
})
...
test("validateEnforcementGuard: accepts a valid guard block", () => {
  let dict: dict<JSON.t> = %raw(`{guard: {budget: 100, blockScriptWrites: true}}`)
  assertionNoThrow(~operator="valid guard", () =>
    ValidateEnforcement.validateEnforcementGuard(dict)
  )
})
...
test("validateEnforcementGuard: accepts guard with all optional fields", () => {
  let dict: dict<
    JSON.t,
  > = %raw(`{guard: {readDraftCap: 5, sameOpRetryCap: 3, blockSelfScript: true, deliverableFirst: false, budget: 500, blockScriptWrites: false}}`)
...
test("validateEnforcementGuard: passes silently when guard is absent", () => {
  let dict: dict<JSON.t> = %raw(`{mode: "off"}`)
  assertionNoThrow(~operator="absent guard", () =>
    ValidateEnforcement.validateEnforcementGuard(dict)
  )
})
