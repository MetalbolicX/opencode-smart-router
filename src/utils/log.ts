...
// session-scoped log lines to `<tmpdir>/opencode-smart-router-trajectory/`.
...
const TRAJECTORY_DIR = "opencode-smart-router-trajectory";

/**
 * Append `content` to a session-scoped trajectory log under
 * `<tmpdir>/opencode-smart-router-trajectory/<sid>[.<subdir>].log`. The
 ...
 */
export const writeTrajectoryLog = (sid: string, content: string, subdir?: string): void => {
  try {
