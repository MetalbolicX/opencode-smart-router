...
test("nextTierAfter: fast => medium", () => {
  let p = makePolicy()
  assertionEqual(~operator="fast", Ladder.nextTierAfter("fast", p)->Nullable.toOption, Some("medium"))
})

test("nextTierAfter: medium => heavy", () => {
  let p = makePolicy()
  assertionEqual(~operator="medium", Ladder.nextTierAfter("medium", p)->Nullable.toOption, Some("heavy"))
})
...
test("nextAction: verdict.pass=true => accept", () => {
  ...
  let a = Ladder.nextAction(s, Nullable.make(({pass: true, reasons: None} : Ladder.ladderVerdict)), p, None)
...
test("nextAction: maxTotalAttempts reached => give_up with message", () => {
  ...
  let a = Ladder.nextAction(s, Nullable.make(({pass: false, reasons: None} : Ladder.ladderVerdict)), p, None)
...
test("nextAction: cost ceiling at exact threshold is NOT exceeded", () => {
  let p = makePolicy(~costMultiple=Nullable.make(2), ~maxTotalAttempts=10, ())
  let s = makeState(~totalAttempts=2, ~firstAttemptCost=Nullable.make(5), ~cumulativeCost=10, ~attemptsThisTier=0, ())
  let a = Ladder.nextAction(s, Nullable.make(({pass: false, reasons: None} : Ladder.ladderVerdict)), p, None)
...
test("nextAction: give_up at top of ladder", () => {
  ...
  let a = Ladder.nextAction(s, Nullable.make(({pass: false, reasons: None} : Ladder.ladderVerdict)), p, None)
  assertionEqual(~operator="action", a.action, #give_up)
  assertionEqual(~operator="reason", a.reason->Nullable.toOption, Some("no higher tier (already at top of ladder)"))
})
...
test("nextAction: costMultiple=0 => cost check never triggers", () => {
  let p = makePolicy(~costMultiple=Nullable.make(0), ~maxTotalAttempts=10, ~maxAttemptsPerTier=1, ())
  let s = makeState(~totalAttempts=2, ~attemptsThisTier=0, ~firstAttemptCost=Nullable.make(1), ~cumulativeCost=99999, ())
  let a = Ladder.nextAction(s, Nullable.make(({pass: false, reasons: None} : Ladder.ladderVerdict)), p, None)
  assertionFalse(~operator="notGiveUp", a.action === #give_up)
})

test("nextAction: firstAttemptCost null => cost check never triggers", () => {
  let p = makePolicy(~costMultiple=Nullable.make(2), ~maxTotalAttempts=10, ~maxAttemptsPerTier=3, ())
  let s = makeState(~totalAttempts=1, ~attemptsThisTier=0, ~firstAttemptCost=Nullable.null, ~cumulativeCost=100, ())
  let a = Ladder.nextAction(s, Nullable.make(({pass: false, reasons: None} : Ladder.ladderVerdict)), p, None)
  assertionFalse(~operator="notGiveUp", a.action === #give_up)
})
...
test("advance: retry increments attemptsThisTier only", () => {
  ...
  let action: Ladder.ladderAction = {action: #retry, tier: Nullable.make("fast"), forcingMessage: Nullable.null, reason: Nullable.null}
...
test("advance: escalate updates currentTier resets attemptsThisTier increments escalations", () => {
  ...
  let action: Ladder.ladderAction = {action: #escalate, tier: Nullable.make("medium"), forcingMessage: Nullable.null, reason: Nullable.null}
  let s2 = Ladder.advance(s, action)
...
test("advance: accept => state unchanged", () => {
  ...
  let action: Ladder.ladderAction = {action: #accept, tier: Nullable.null, forcingMessage: Nullable.null, reason: Nullable.null}
  let s2 = Ladder.advance(s, action)
...
test("advance: give_up => state unchanged", () => {
  ...
  let action: Ladder.ladderAction = {action: #give_up, tier: Nullable.null, forcingMessage: Nullable.null, reason: Nullable.make("done")}
  let s2 = Ladder.advance(s, action)
  assertionEqual(~operator="same", s2, s)
})
...
test("edge: retry then PASS => accept on second call", () => {
  ...
  let a1 = Ladder.nextAction(s2, Nullable.make(({pass: false, reasons: None} : Ladder.ladderVerdict)), p, None)
  assertionEqual(~operator="retry1", a1.action, #retry)
  let s3 = Ladder.advance(s2, a1)
  let s4 = Ladder.recordAttempt(s3, ~costUnits=1)
  let a2 = Ladder.nextAction(s4, Nullable.make(({pass: true, reasons: None} : Ladder.ladderVerdict)), p, None)
