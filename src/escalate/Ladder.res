...
// NOTE: defaultTierNames removed - unused
let _defaultTierNames = TierLadder.defaultTierNames
    ...
    if state.totalAttempts >= policy.maxTotalAttempts {
      {
        ...
        tier: Nullable.null,
        forcingMessage: Nullable.null,
        reason: Nullable.make(
          "max total attempts (" ++ Belt.Int.toString(policy.maxTotalAttempts) ++ ") reached",
        ),
      }
      ...
      // (5) cost ceiling
      {
        ...
        tier: Nullable.null,
        forcingMessage: Nullable.null,
        reason: Nullable.make("cost ceiling exceeded"),
      }
      ...
      {
        action: #retry,
        tier: Nullable.make(state.currentTier),
        forcingMessage: Nullable.make(buildLadderForcingMessage(reasons)),
      ...
      | None => {
          ...
          tier: Nullable.null,
          forcingMessage: Nullable.null,
          reason: Nullable.make("no higher tier (already at top of ladder)"),
        }
      | Some(next) => {
          let reasons = switch verdictOpt {
          | Some(v) => v.reasons->Belt.Option.getWithDefault([])
...
let formatLadderScorecard = (state: ladderState, accepted: bool, method: string): string => {
  ...
  "[router delegate scorecard | final_tier=" ++
  state.currentTier ++
  " | attempts=" ++
  Belt.Int.toString(state.totalAttempts) ++
  " | escalations=" ++
  Belt.Int.toString(state.escalations) ++
  " | cost=" ++
  Belt.Int.toString(state.cumulativeCost) ++
  " | verdict=" ++
  verdictStr ++
  " | method=" ++
  method ++ "]"
